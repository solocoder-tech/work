...
int
i420ToRgb24(JNIEnv *env, jclass clazz, jbyteArray yuv, jint y_stride, jint u_stride, jint v_stride,
            jbyteArray rgba, jint rgba_stride, jint width, jint height,
            int (*func)(const uint8 *, int, const uint8 *, int, const uint8 *, int, uint8 *,
                        int, int, int)) {
    size_t ySize = (size_t) (y_stride * height);
...
int Jni_I420ToNV21(JNIEnv *env, jclass clazz, jbyteArray yuv420p, jbyteArray yuv420sp, jint width,
    ...
    int ret = libyuv::I420ToNV21((const uint8 *) yuv420pData, width,
                                 ...
                                 (const uint8 *) (yuv420pData + ySize + stride[1 - swapUV]),
                                 width >> 1,
                                 (uint8 *) yuv420spData, width, (uint8 *) (yuv420spData + ySize),
                                 width, width, height);
...
int Jni_NV21ToI420(JNIEnv *env, jclass clazz, jbyteArray yuv420p, jbyteArray yuv420sp, jint width,
    ...
    jbyte *yuv420pData = env->GetByteArrayElements(yuv420p, JNI_FALSE);
    jbyte *yuv420spData = env->GetByteArrayElements(yuv420sp, JNI_FALSE);
    int ret = libyuv::NV21ToI420((const uint8 *) yuv420spData, width,
                                 (const uint8 *) (yuv420spData + ySize), width,
                                 (uint8 *) yuv420pData, width,
                                 (uint8 *) (yuv420pData + ySize + stride[swapUV]), width >> 1,
                                 (uint8 *) (yuv420pData + ySize + stride[1 - swapUV]), width >> 1,
                                 width, height);
    env->ReleaseByteArrayElements(yuv420p, yuv420pData, JNI_OK);
    env->ReleaseByteArrayElements(yuv420sp, yuv420spData, JNI_OK);
    return ret;
...
Jni_NV21Scale(JNIEnv *env, jclass clazz, jbyteArray src, jint width, jint height, jbyteArray dst,
    ...
    libyuv::ScalePlane((const uint8 *) srcData, width, width, height,
                       (uint8 *) dstData, dst_width, dst_width, dst_height,
...
Jni_I420Scale(JNIEnv *env, jclass clazz, jbyteArray src, jint width, jint height, jbyteArray dst,
    ...
    libyuv::I420Scale((const uint8 *) srcData, width, (const uint8 *) (srcData + ySize), width >> 1,
                      ...
                      (uint8 *) dstData, dst_width, (uint8 *) (dstData + dstYSize + swap[swapUV]),
                      dst_width >> 1,
...
void Jni_NV21ToI420Rotate(JNIEnv *env, jclass clazz, jbyteArray src, jint width, jint height,
    ...
    jbyte *srcData = env->GetByteArrayElements(src, JNI_FALSE);
    jbyte *dstData = env->GetByteArrayElements(dst, JNI_FALSE);
    rotatePlaneFunc[de]((const uint8 *) srcData, width, (uint8 *) dstData, dst_stride[de], width,
                        height);
    rotateUVFunc[de]((const uint8 *) (srcData + ySize), width,
                     (uint8 *) (dstData + ySize + swap[swapUV]), dst_stride[de] >> 1,
                     (uint8 *) (dstData + ySize + swap[1 - swapUV]), dst_stride[de] >> 1,
                     width >> 1, height >> 1);
    env->ReleaseByteArrayElements(src, srcData, JNI_OK);
    env->ReleaseByteArrayElements(dst, dstData, JNI_OK);
}
...
int Jni_I420ToRgb24(JNIEnv *env, jclass clazz, jint type, jbyteArray yuv, jbyteArray rgb24, jint width,
                jint height) {
    uint8 cType = (uint8) (type & 0x0F); //4
    int rgba_stride = ((type & 0xF0) >> 4) * width;
    int y_stride = width;
    int u_stride = width >> 1;
    int v_stride = u_stride;
    LOGE("cType === %d", cType);
    return i420ToRgb24(env, clazz, yuv, y_stride, u_stride, v_stride, rgb24, rgba_stride, width,
                       height, i420ToRgbaFunc[cType]);
}
...
static JNINativeMethod g_methods[] = {
        ...
        {"I420ToRgb24",      "(I[B[BII)I",     (void *) Jni_I420ToRgb24},
